...
class JvsoabController extends AdminController {
    ...
    public function getIndex()
    {
        ...
        $this->additional_filter = View::make(strtolower($this->controller_name).'.addfilter')
                ->with('maxperiod',$period_from)
                ->with('submit_url','jvsoab')->render();
                ...
                */
                ...
                foreach ($md as $mk => $m) {
                    ...
                    $movement = $m->CMITDAMT - $m->PYAMT;
                        ...
                        $sum->PMAMT += (float) $m->PMAMT;
                        $sum->AMT += (float) $m->AMT;
                        $sum->CMITDAMT += (float) $m->CMITDAMT;
                        $sum->PYAMT += (float) $m->PYAMT;
                        $sum->movement += (float) $movement;
                ...
                $tdata[] = array(
                        ...
                        array('value'=>floatval($sum->AMT), 'attr'=>'class="column-amt total"' ),
